...
| `PI_MEMORY_EXIT_SUMMARY` | `0`, `off`, `false`, `no` to disable; `detached` (alias `background`) for background mode | unset (enabled, inline) | Controls the exit summary on real quit (Ctrl+D, `/quit`, session end). Disable: quitting does no LLM call and no `qmd update`, so it is instant; explicit `memory_write` during sessions is unaffected. `detached`: the summary is handed to a detached worker process over a one-shot stdin pipe (the API key and conversation payload are never written to disk), so quitting is instant too, while the summary still lands in the daily log (and `qmd update` runs) after pi has already exited — `PI_MEMORY_EXIT_SUMMARY_TIMEOUT_MS` does not apply in this mode. Default is the inline behavior: the summary is generated during shutdown, bounded by the timeout below. |
...
| `PI_MEMORY_EXIT_SUMMARY_TIMEOUT_MS` | positive integer (milliseconds) | `10000` | Self-imposed timeout for exit-summary generation on quit (inline mode only). Pi awaits shutdown handlers with no timeout, so a hanging provider would otherwise block quitting indefinitely. On expiry nothing is persisted. Irrelevant in `detached` mode, where the worker runs after pi has exited. |
